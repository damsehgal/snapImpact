...
<!--map and side navigation-bar-ends-->
	...
	$count="select count('issueid') from issues WHERE status='0'";
        ...
        $alat=array();
        $alng=array();
        $query1="select world_cities_table.lat from world_cities_table, issues where issues.status='0' AND issues.type='Garbage Issues' ORDER BY ABS(world_cities_table.lat-issues.location_latitude) + ABS (world_cities_table.lng-issues.location_longitude)";
        $query2="select world_cities_table.lng from world_cities_table, issues where issues.status='0' AND issues.type='Garbage Issues' ORDER BY ABS(world_cities_table.lat-issues.location_latitude) + ABS (world_cities_table.lng-issues.location_longitude)";
